...
class GffTranscriptReader:
    ...
    def computeFrames(self, transcripts):
        for transcript in transcripts:
            if not transcript.areExonTypesSet():
    ...
    def adjustStartCodons_fw(self, transcript, totalIntronSize):
        ...
        for i in range(numExons):
            ...
            if transcript.startCodon is not None and self.exonContainsPoint(
                exon, transcript.startCodon
            ):
                break
        return startCodon
    ...
    def loadGFF_transcript(
        self, fields, line, transcriptBeginEnd, GFF, transcripts, readOrder, genes
    ):
        ...
        if rex.find('transcript_id[:=]?\s*"?([^\s";]+)"?', line):
            ...
            if transcript is None:
                transcripts[transcriptId] = transcript = Transcript(
                    transcriptId, strand
                )
            ...
            if transcript.score is None and score != ".":
                transcript.score = float(score)
            geneId = None
            if rex.find("genegrp=(\S+)", line):
                geneId = rex[1]
            elif rex.find('gene_id[:=]?\s*"?([^\s\;"]+)"?', line):
                geneId = rex[1]
            if not geneId:
                raise Exception("can't parse GTF: " + line)
            transcript.geneId = geneId
            gene = genes.get(geneId, None)
            if not gene:
                genes[geneId] = gene = Gene()
                gene.setId(geneId)
    ...
    def loadGFF_UTR(
        ...
        transcriptId = None
        rex = Rex()
        if rex.find("transgrp[:=]\s*(\S+)", line):
            transcriptId = rex[1]
        elif rex.find('transcript_id[:=]?\s*"?([^\s";]+)"?', line):
            transcriptId = rex[1]
        elif rex.find("Parent=([^;,\s]+)", line):
            transcriptId = rex[1]
        geneId = None
        if rex.find("genegrp=(\S+)", line):
            geneId = rex[1]
        ...
        if rex.find("(\S+);$", transcriptId):
            transcriptId = rex[1]
        if rex.find("(\S+);$", geneId):
            geneId = rex[1]
        extra = ""
        for i in range(8, len(fields)):
            extra += fields[i] + " "
        ...
        transcript = transcripts.get(transcriptId, None)
        if not transcript:
            transcripts[transcriptId] = transcript = Transcript(transcriptId, strand)
            transcript.setStopCodons(self.stopCodons)
            transcript.readOrder = readOrder
            readOrder += 1
            transcript.substrate = fields[0]
    ...
    def loadGFF_exon(
        ...
        exon.score = exonScore
        exon.type = fields[2]
        if transcript.rawExons is None:
            transcript.rawExons = []
        transcript.rawExons.append(exon)
        gene.addTranscript(transcript)
    ...
    def loadGFF(self, gffFilename):
        if ".gz" in gffFilename:
            GFF = gzip.open(gffFilename, mode="rt")
        else:
        ...
        while True:
            ...
            if fields[2] == "transcript" or fields[2] == "mRNA":
                self.loadGFF_transcript(
                    fields, line, transcriptBeginEnd, GFF, transcripts, readOrder, genes
                )
        ...
        if self.shouldSortTranscripts:
            transcripts.sort(
                key=lambda t: t.substrate + " " + str(t.begin) + " " + str(t.end)
            )
        else:
